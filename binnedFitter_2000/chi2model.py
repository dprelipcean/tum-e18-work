...
class DalitzChi2model:
	def __init__(self, m_mother, fs_masses, _bins, mesh_width, functions, n_term_in_bin=10):
		"""
		@param m_mother    mass of the mother particle
		@param fsMasses   list of final state masses: [m1, m2, m3] with m12**2 on the X-axis and m13**2 on the Y-axis
		@param _bins       list of bins to use. Format [(m12_low, m12_high, m13_low, m13_high), ... ]
		@param mesh_width  distance of two points in one direction on the integral mesh
		@param functions  list of functions ( = partialWaves)
		@param n_term_in_bin (estimated) number of non-zero terms in every bin ( = nonZeroAmpls**2)
		"""
	...
	def make_hist(self, data, tag=''):
		...
		hist = ROOT.TH2D(f"Dalitz{tag}", f"Dalitz{tag}", nx - 1, nx, ny - 1, ny)
